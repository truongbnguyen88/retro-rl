...
- **Training run v3 in background** — `python scripts/train.py --config configs/ppo.yaml` → `ppo_airstriker_v3`, 2M steps, `n_envs=8` SubprocVecEnv. Launched 2026-05-18 after terminating v2 at 1.66M steps. v2 stochastic policy was strong (rollout 1174) but the deterministic eval froze at -35.75 for 700K consecutive steps — root cause was the `MultiBinary(12)` action space's Bernoulli threshold (P(B=1) ≈ 0.14 in stochastic exploration, never crossing 0.5 for deterministic argmax). v3 swaps in a `Discrete(9)` always-fire action space via the new `DiscreteActionWrapper`; the policy now uses a single categorical head where firing is unconditional and only movement is learned. v1 + v2 artifacts preserved under `outputs/checkpoints/ppo_airstriker{,_v2}/`. TB log at `outputs/tensorboard/ppo_airstriker_v3_1`.
