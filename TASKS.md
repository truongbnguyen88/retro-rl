...
- **Training run v7 in background** — `python scripts/train.py --config configs/ppo.yaml` → `ppo_airstriker_v7`, 2M steps, seed=42, `n_envs=8` SubprocVecEnv. Launched 2026-05-19 after killing v6 at 1.6M. Tests the hypothesis that AutoFire `period=24` (2.5 Hz — slow enough that Airstriker's bullet sprite array never saturates) keeps fire visibly active throughout each life, which should let the agent survive deeper into Level 1 instead of dying in the first ~5s like v5/v6 did. Carries forward v6's linear `ent_coef` schedule (0.02 → 0.001 over total_timesteps). v1–v6 artifacts preserved for ablation comparison. TB log at `outputs/tensorboard/ppo_airstriker_v7_1`.
- Milestone 5 landed. Milestone 6 (Streamlit frontend) still queued after v7 finishes.

## Next up (queue)

1. Evaluate v7 at 2M and compare head-to-head vs v5 (mean 193 / peak 244 over 2M) and v6 (mean 222 / peak 275 over 1.6M). If v7's `eval/mean_length` doesn't climb significantly above 500 outer steps by 1M, the slot-saturation theory under-explained the plateau and we'd try period=12 as a middle ground.
2. Fix the eval bug: `eval_std=0.00` across every checkpoint because the env is deterministic given the Level1 save state and the policy is deterministic in eval, so all 5 eval episodes replay one trajectory. Vary seeds per episode in `EvalAndVideoCallback` and/or set `deterministic=False`. Orthogonal to v7's outcome.
3. Milestone 6 — frontend (Streamlit dashboard against the now-live backend)
4. Milestone 7 — polish (docs, CI, pre-commit, optional `docs/api.md`)
