...
- **Training run v2 in background** — `python scripts/train.py --config configs/ppo.yaml` → `ppo_airstriker_v2`, 2M steps, `n_envs=8` SubprocVecEnv, ~207 fps → ETA ~2.7h wall. Re-launched 2026-05-18 after killing v1 at 665K steps; v1's deterministic policy collapsed to "move left → die" (confirmed visually from eval videos at 500K/600K/700K) and produced `eval/mean_return = -10.0` with `std=0.0` across all checkpoints. v2 uses corrected reward shaping (see Decisions log, 2026-05-18 entry on shaping v2) and `ent_coef: 0.02`. v1 artifacts preserved under `outputs/checkpoints/ppo_airstriker/` for comparison.
