...
See [TASKS.md](TASKS.md) for milestone tracking. **Milestones 1–7 complete** (env layer, models, agents, trainer, evaluator, FastAPI backend, Streamlit frontend, docs + CI + pre-commit). Experimental training iteration complete.

| Run | Algorithm | Best return | Status |
|-----|-----------|-------------|--------|
| v8 | PPO + RetroCNN | 4271 @ 3.3M | ✅ complete |
| v9 | PPO + IMPALA ResNet + VecNormalize | 7168 @ 2.7M | ✅ complete — clears the game |
...
| v12 | PPO + **temporal-attention** extractor, frame_stack=8 | **7609 @ 5.3M** | ✅ **new best (+6.2% vs v9)** |

**Experiment complete.** v10/v11 tested whether an LSTM's recurrent memory could beat the stateless v9 baseline — both fell short, capped by the LSTM's `(h, c)=0` cold-start at every episode reset. v12 swaps the LSTM for **self-attention over the K=8 frame window** ([`models/attention.py`](src/retro_rl/models/attention.py)) and runs on **standard PPO** — no hidden state, no cold-start. At 6M steps it reached **7609** (new best), exceeding v9 by 6.2%. Trade-off: v12 needs 2× the steps of v9 and has higher eval variance (one 5.9M collapse to 2212); v9 remains the compute-efficiency champion. See the TASKS.md decisions log for the full rationale.
...
8. **Transformers need more steps than CNNs to converge, but they do converge.** v12's two 500K smokes both landed 39% below v9's smoke bar — a plateau at 200K-300K that looks like failure but is actually a transformer optimization cold-start (attention heads take longer to discover useful frame-to-frame relationships than a CNN's local filters). At 6M steps the architecture fully converged and exceeded v9 (+6.2%). The lesson: a smoke that looks bad for a transformer is not automatically a GO/NO-GO signal — check whether the return curve is still climbing steeply at the cutoff (it was) before deciding to abort. The 2× sample cost vs a comparably-performing CNN is a genuine trade-off: compute-efficiency still favors the simpler IMPALA architecture (v9 gets to within 6% at half the steps).
